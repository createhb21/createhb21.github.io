...
프론트엔드 개발자라면 누구나 한 번쯤 고민해봤을 주제다.
"Cypress 쓸까, Playwright 쓸까?"
혹은 "지금 쓰는 도구를 바꿔야 하나?"

특히 **CI에서 테스트가 랜덤하게 깨지는** 경험이 반복되면
'이게 정말 맞는 도구인가?'라는 의문이 생기기 마련이다.

오늘 함께 읽어보려는 글은,
실제로 **500개의 E2E 테스트**를 Cypress에서 Playwright로 옮긴 개발자의 경험담이다.
단순 비교가 아니라 실제로 뭐가 깨졌고 뭐가 좋아졌는지 솔직하게 풀어낸다.
...
두 달 전, 우리 팀 QA 리드가 그만뒀다.
슬랙에 "flaky 테스트들 잘 해결하세요 ㅎ" 메시지만 남기고 사라졌다.

> `flaky`는 실행 환경이나 타이밍 문제로 결과가 오락가락하는, 신뢰할 수 없는 테스트(Flaky Test)를 의미한다.

우리한테는 Cypress로 작성된 500개의 E2E 테스트가 있었다.
절반은 랜덤하게 실패했고,
나머지 절반은 돌리는 데만 45분이 걸렸다.
즉, CI 파이프라인은 재앙에 가까웠다.

그래서 테스트 인프라가 불타는 상황에서,
합리적인 사람이라면 누구나 할 법한 일을 했다.
**Playwright로 전부 다시 짜보기로.**
...
우리 앱은 꽤 평범한 SaaS 제품이다.
사용자 대시보드, 데이터 테이블, 폼, 파일 업로드.
딱히 특별할 건 없는 구성이다.
...
모든 PR이 테스트를 돌렸다.
그리고 매번 5~8개가 랜덤하게 실패했다.
실패할 때마다 파이프라인 전체를 다시 돌려야 했다.

개발자들은 테스트를 안 믿게 됐다.
실패가 떠도 어깨 한번 으쓱하고,
내용도 안 보고 "재실행" 버튼을 누르곤 했다.
...
**Cypress는 문제를 숨겼다.**
빌트인 재시도 로직, 자동 대기, 타이밍 이슈를 가리는 타임아웃.
실패해도 다시 시도해서 결국 “통과”시켜버렸다.
문제가 있었는지조차 모른 채로.

**Playwright는 문제를 드러냈다.**
자동 재시도도, 마법 같은 대기도 없다.
테스트에 레이스 컨디션이 있으면 실패한다.
매번, 시끄럽게.
...
이 패턴이 수십 개 테스트에서 반복됐다.
결과적으로 **Playwright는 더 나은 테스트를 쓰게 만들었다.**
...
CI 파이프라인이 다시 신뢰를 회복했다.
개발자들도 무지성 재실행 대신 실패한 테스트를 실제로 읽기 시작했다.
...
타임 트래블 디버거, 자동 스크린샷,
무엇이 어떤 순서로 일어났는지 보여주는 커맨드 로그까지.
솔직히 말하면 아름답다.

Playwright 디버깅은... 괜찮다.
실패하면 스크린샷이 남고, `page.pause()`로 단계별 실행도 가능하다.
하지만 같은 느낌은 아니다.

Cypress에서는 실패하면 “아, 여기서 이렇게 됐구나”가 바로 보인다.
Playwright에서는 실패 로그와 스크린샷을 보고 상황을 추론해야 한다.
...
우리는 전체 테스트 스위트를 세 브라우저에서 모두 돌린다.
그 과정에서 Cypress로 놓쳤던 브라우저별 버그를 실제로 잡아냈다.
...
API 테스트와 E2E 테스트를 같은 스위트에서 섞어 돌릴 수 있다.
컨텍스트도 공유된다.
생각보다 훨씬 유용하다.
...
**다만 Cypress 테스트가 이미 빠르고 안정적이라면, 굳이 지금 마이그레이션할 이유는 없다.**
...
이 차이가 우리가 미처 몰랐던 버그를 잡아냈다.
기술적으로는 보이지만, 로딩 스피너에 가려져 있던 버튼 같은 것들 말이다.
...
하지만 여전히 문제가 있다.
E2E 테스트는 유닛 테스트보다 느리고, 유지보수가 어렵고, UI가 바뀌면 쉽게 깨진다.

**진짜 교훈은 이것이다. E2E 테스트를 더 적게 써라.**

우리가 500개 테스트를 가지게 된 이유는 단순했다.
무언가 깨질 때마다 테스트를 계속 추가했기 때문이다.
오래된 테스트는 지우지 않았고, 정말 필요한지도 점검하지 않았다.

마이그레이션 이후 **200개를 지웠다.**
중복된 것, 오래된 플로우, 유닛 테스트가 더 잘 커버하는 것들이었다.
...
E2E 테스팅은 퍼즐의 일부일 뿐이다.
브라우저 자동화와 싸우지 않을 때는,
Spring Boot 서비스가 왜 RAM을 잡아먹는지,
로컬에서 되던 DB 쿼리가 왜 프로덕션에서 죽는지 디버깅하고 있다.

백엔드 테스트와 운영 문제를 겪고 있면, 내가 계속 발견한 패턴을 바탕으로 참고 자료를 작성했다.

백엔드 실패 플레이북은
실제 시스템에서 어떤 문제가 생기는지,
운영에서 터지기 전에 어떻게 감지할지에 초점을 맞춘 자료다.
이론보다는 운영에서 겪는 문제 중심으로 정리되어 있다.
...
실제로는 팀 상황마다 답이 달라진다.
Cypress를 쓰든 Playwright를 쓰든,
어떤 고통을 줄이고 싶은지부터 정하면 선택이 훨씬 쉬워진다.

마이그레이션을 고민 중이라면
먼저 현재 테스트 케이스부터 분석해보자.
마이그레이션이 필요 없을 수도 있다.
...
나는 수년간 실제 프로덕션 시스템을 구축하고 운영해온 엔지니어이자 창업가다.
시스템이 터질 때 무슨 일이 벌어지는지도 직접 겪어왔다.

새벽에 터지는 장애, 불분명한 근본 원인, 위험한 릴리즈.
팀에서 한두 명만 이해하는 시스템까지 전부 경험했다.
