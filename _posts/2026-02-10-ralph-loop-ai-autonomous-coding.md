...
사람이 프롬프트를 한 번도 입력하지 않아도 코딩 루프가 계속 돌아갈 수 있을까? 만약 가능하다면, 우리는 검수와 방향 제시에 집중하고 반복 지시는 자동화할 수 있다. Ralph Loop은 바로 그 가설을 CLI 환경에서 밀어붙인 실험이다.

직접 세팅하고 돌려보니 병목이 어디서 줄고 어디서 다시 생기는지 선명하게 드러났다. 아이디어를 잡았으니, 먼저 Ralph Loop의 구조부터 짧게 펼쳐보자.
...
처음부터 구현을 돌리면 우선순위가 뒤엉켜서 같은 파일을 여러 번 되돌리는 일이 잦았다. 그래서 먼저 AI가 코드베이스를 읽고 작업 순서를 정리하게 만들었다.
...
계획 검토를 거치고 나서야 빌드를 돌리는 이유는, 반복 루프가 길어질수록 초기 방향 오류가 비용으로 커지기 때문이다.
...
이제 AI가 매 반복마다 핵심 항목을 집어 구현하고, 테스트를 통과하면 커밋과 푸시까지 진행한 뒤 계획 파일을 갱신한다. 이미 처리된 항목은 코드 확인 단계에서 건너뛰기 때문에, 같은 일을 다시 시키는 빈도가 확실히 줄어들었다.
...
2. Claude Code가 작업을 진행하다 종료 시점에 도달하면 Stop 훅이 세션을 붙잡는다
3. 같은 프롬프트가 다시 주입되고, completion promise 출력 또는 최대 반복 횟수까지 같은 패턴이 이어진다
...
`--completion-promise` 문구를 너무 일반적인 단어로 잡으면 작업 중간 로그에도 같은 문자열이 섞여서 루프가 조기 종료되는 경우가 있었다. 나는 `<promise>COMPLETE</promise>`처럼 충돌 가능성이 낮은 토큰을 써서 오탐을 줄였고, 장시간 실행에는 `--max-iterations`를 함께 걸어 무한 루프 리스크를 제한했다.
