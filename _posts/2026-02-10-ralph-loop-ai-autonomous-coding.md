...
**새벽에 돌려놓고 아침에 확인하는 패턴이 가능하다.** `./loop.sh 30`을 걸어두고 자면, 아침에 커밋 히스토리를 보면서 "여기까지 진행됐구나"를 확인할 수 있다.

**git 히스토리가 깔끔하다.** 매 작업마다 커밋이 찍히니까 뭘 했는지 추적이 쉽다. 문제가 생기면 해당 커밋만 revert하면 된다.

### 주의할 점

**비용.** 여기는 꼭 확인해야 한다. Opus 모델로 루프를 돌리면 반복당 API 비용이 만만치 않다. `max_iterations`를 설정하지 않고 무제한으로 돌리면 예상보다 비용이 커질 수 있다. 반복 횟수 제한은 필수다.
...
적어도 지금 내 기준에서는, 그 변화를 가장 직접적으로 체감하게 해준 방법이 Ralph Loop였다.
