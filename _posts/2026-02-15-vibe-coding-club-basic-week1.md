...
바이브 코딩 클럽은 바로 이 지점에서 출발한다. AI 에이전트가 뭔지, 왜 지금 중요한지, 그리고 어떻게 시켜야 제대로 일하는지를 다뤄보려 한다.
...
이 프롬프트에서 가장 중요했던 건 **Chain of Thought로 단계별 사고 로직을 명시한 것**과, **Output Format으로 결과물 형태를 미리 못 박아둔 것**이었다. 덕분에 AI가 대충 넘기지 않고 검증까지 하게 만들 수 있었다.
