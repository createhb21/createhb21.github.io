...
오늘 실습에서 제일 인상적이었던 건, 같은 질문도 프롬프트 틀을 어떻게 잡느냐에 따라 답변의 무게가 완전히 달라졌다는 점이었다. 특히 회의 시간 잡기 같은 문제는 그냥 물으면 얼버무리는데, 단계와 출력 형식을 박아두니 결과가 훨씬 단단해졌다.
중간에 나도 "이 정도면 되겠지" 하고 컨텍스트를 짧게 넣었다가, 모델이 엉뚱한 전제를 깔고 답해서 다시 처음부터 작성했다. 그때 체감했다. 에이전트를 잘 쓰는 건 말을 예쁘게 하는 기술이 아니라, 일을 시키는 사람의 책임감에 더 가깝다.
다음 주에는 n8n으로 실제 자동화 흐름을 만들면서, 오늘 배운 구조화 프롬프트가 어디까지 통하는지 직접 비교해볼 생각이다.
